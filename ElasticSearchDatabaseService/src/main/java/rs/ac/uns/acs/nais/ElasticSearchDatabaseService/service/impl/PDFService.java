...
public class PDFService {
    ...
    public byte[] generatePDFBasedOnItemDescription(String desc) {
        Document document = new Document(PageSize.A4, 50, 50, 50, 50); // Set document margins
        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
        ...
        try {
            ...
            // Adding a title
            Paragraph title = new Paragraph("Locals that have items that they contain in the description  " + desc, titleFont);
            title.setAlignment(Element.ALIGN_CENTER);
            title.setSpacingAfter(20);
            document.add(title);

            List<Local> locals = localService.findLocalsByItemDescription(desc);
